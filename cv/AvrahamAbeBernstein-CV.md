...
4. [Zig](https://ziglang.org/): new `C`-like programming language that can cross compile to `C` and that conforms to the industry standard `C` [Application Binary Interface (ABI)](https://en.wikipedia.org/wiki/Application_binary_interface). Has many new language features such as much better dynamic memory allocation mechanism, generic types, and compile time code instead of preprocessing macros.
