...
<br/>__Creative Commons Copyright:__ [CC BY](https://creativecommons.org/licenses/by/4.0/)
...
In general I find that running a pre-processing parser that _understands the structure of the raw text_ used to "prompt" an LLM provides much better results than simply feeding an LLM raw text. For example a common problem is using an LLM to process a PDF file. The processing can be greatly improved when fed the Markdown text that was used to generate the PDF. When the original Markdown is not available then a FOSS program such as the _markdown-it_ PDF parser which outputs Markdown enables the LLM to generate much more accurate and much less expensive results because the raw PDF file contains a huge amount of formatting "noise" which intentionally is not contained in the Markdown file. Similarly an LLM can understand computer source code far better when provided with its _AST_ description (e.g. output by _SrcML_ or _Tree-Sitter_) as opposed to the raw source code. And very recently (June 2026) Google has invented a v0.1 public domain [Open Knowledge Format (OKF)]( https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing/ ) language for describing an LLM Wiki comprised of human readable (industry standard) Markdown and YAML files that makes it more accurate and faster and more economical for the various LLM engines to contextualize knowledge. I am starting an _OKF_ experiment using the _Obsidian_ (2nd brain) wiki package combined with an _Ollama_ local LLM.
...
3. _FOSS_ [Tree-Sitter](https://tree-sitter.github.io/tree-sitter/) [Abstract Syntax Tree (AST)](https://en.wikipedia.org/wiki/Abstract_syntax_tree): dynamic parser plugin for programming editors such as _neovim_ (above).
...
7. [NeuroSky](https://neurosky.com/): a small inexpensive (~ USD\$200) **non-invasive** brain computer interface (BCI) that enables brain waves to control a USB keyboard and mouse. Physically the device is similar to a plastic woman's hair band worn across the forehead. Especially important their patent does not require pasting any electrodes on the scalp. It is to be expected that many users will _not_ be capable of generating the minimum of 64 distinct keys available on a full keyboard; but most certainly they could generate say 8 distinct signals, therefore a "chording" macro could easily be implemented that combined 2 or 3 signals to generate a single virtual key. It is an important inexpensive accessibility device for those who have lost use of their hands. Unfortunately here in Israel there are hundreds of soldiers who have recently lost of use of their hands with no cognitive impairment. The rehabilitation centers here would provide an ideal environment to refine such an invention.
