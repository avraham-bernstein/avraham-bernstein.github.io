...
5. Use of the '@' character that immediately precedes a new word triggers a pandoc citation.
I don't know how to disable this facility, so escape them by using a backslash, i.e. '\@'.
6. The current Python2 version of j2-cli.py does not support UTF-8 or Unicode,
so make sure the document is written in ISO-8859-1.
#}

%# includes 'defs.j2' from the j2-cli.py command line

# {{TITLE}} { #top }

%% if AGENT_URL != ''
{% if WRITER == 'html5' %}<span style="font-size:larger;background-color:#FFFF00;">{% endif %}__Represented by [{{AGENT_NAME}}]({{AGENT_URL}})__{% if WRITER == 'html5' %}</span>{% endif %}
%% endif
{{NL}}

%% if FULL or AGENT != 'wizedom'
_Version: {{VS}}_
{{BR}}_Last update: {{TS}}_
%% else
_Last update: {{TS[0:10]}}_
%% endif
{{BR}}_{{COPYRIGHT}}_
{{BR}}_{{LICENSE}}_
...
__email:__ [{{EMAIL}}](mailto:{{EMAIL}}) [^obf-email]
...
{{BR}}__cv-abbrev:__ [HTML]({{CV_HOME}}abbrev.html), [DOCX]({{CV_HOME}}abbrev.docx), [PDF]({{CV_HOME}}abbrev.pdf)
%% if ABBREV
__\[this file]__
%% endif
{{BR}}__cv-full:__ [HTML]({{CV_HOME}}full.html), [DOCX]({{CV_HOME}}full.docx), [PDF]({{CV_HOME}}full.pdf)
%% if FULL
__\[this file]__
%% endif
{{NL}}
[^obf-email]: For security reasons to protect against [email harvesting](https://en.wikipedia.org/wiki/Email_address_harvesting), the email address {% if WRITER == 'html5' %}and telephone numbers have{% else %}has{% endif %} been obfuscated{% if WRITER == 'html5' %} with HTML5 & CSS3 but without the use of Javascript{% endif %}.
...
* [cybersecurity]({% if ABBREV %}{{CV_FULL}}{% endif %}#viaccess)
* [cryptography]({% if ABBREV %}{{CV_FULL}}{% endif %}#viaccess)
* [bioinformatics]({% if ABBREV %}{{CV_FULL}}{% endif %}#syntezza)
* [transportation vehicle route guidance]({% if ABBREV %}{{CV_FULL}}{% endif %}#telequest)
* [factory automation]({% if ABBREV %}{{CV_FULL}}{% endif %}#iscar)
* [automated testing]({% if ABBREV %}{{CV_FULL}}{% endif %}#elop)
* [blind vision]({% if ABBREV %}{{CV_FULL}}{% endif %}#virtouch)
* [accessibility]({% if ABBREV %}{{CV_FULL}}{% endif %}#light-pen)
* [telecommunications]({% if ABBREV %}{{CV_FULL}}{% endif %}#vyyo)
* {% if AGENT == 'arm' %}{{HI_BEGIN}}{% endif %}[VLSI CPU design]({% if ABBREV %}{{CV_FULL}}{% endif %}#dspg){% if AGENT == 'arm' %}{{HI_END}}{% endif %}
...
In order to understand my S/W design principles, see the following appendices{% if ABBREV %} (in the full version of my CV){% endif %}:

* [Programming Language Preferences and Musings]({% if ABBREV %}{{CV_FULL}}{% endif %}#appendix-programming-languages)
* [Domain Specific Languages]({% if ABBREV %}{{CV_FULL}}{% endif %}#appendix-dsl)
* [How To Write Correct, Maintainable, Secure, and Easy-to-Test Code]({% if ABBREV %}{{CV_FULL}}{% endif %}#appendix-programming-testable)


%% if AGENT == 'it'
### 1.1 IT, Cobol, Adabas, Natural, C#, IBM Mainframes, and ETL Programming Credentials

#. {{HI_BEGIN}}This section is admittedly unusual, and may be too difficult for non-technical people to comprehend. So please bear with me. In spite of my best efforts to make it understandable, if you still find it difficult to understand how using my skills would immediately benefit your organization, then please pass my CV on to someone more technical or more senior in your organization who can.{{HI_END}}
#. {{HI_BEGIN}}As you can see in my CV, I have no substantive credentials doing IT work, or working with Cobol, Adabas, Natural, C#, or IBM mainframes.{{HI_END}}
#. {{HI_BEGIN}}However it should be clear that I am an accomplished computers scientist. I am a [polyglot](http://www.dictionary.com/browse/polyglot). I am an expert in many computer languages, and I comfortably work with many more. I write compilers and [domain specific languages (DSL)](https://en.wikipedia.org/wiki/Domain-specific_language) so learning a new language, and being able to work with it at a professional level, usually takes me about a week.{{HI_END}}
#. {{HI_BEGIN}}I am an expert in applying templates and macros to any programming language. These macros and templates allow the underlying language to incorporate the latest programming concepts and features while still emitting valid (and human understandable) source code in the original language.{{HI_END}}
#. Similarly I am very familiar with a number of operating systems.
#. I was the architect of a major soft real-time project in SQL, and I taught a university level course in databases.
#. Regarding ETL, my compiler background allows me to write conversion filters "in my sleep".
#. I am a cybersecurity expert, and therefore have a much better eye for detecting and resolving security issues.
#. I have a strong theoretical background that allows me to engage in conversion projects of legacy S/W and H/W to modern systems. Everyone in the IT business is aware that the major force driving conversions is that the number of programmers who can maintain legacy systems using the original S/W and H/W is dwindling as they reach retirement age, while few quality graduates are willing to consider entering this field because they don't want their CV to show that their only experience is using archaic tools and systems.
#. I have a strong background in automated testing which I have done for telecommunication and military systems.
#. I am also an economist where the application domain of many IT systems is in fact financial. It helps a great deal to have a strong understanding of the underlying application domain.
#. {{HI_BEGIN}}For the rest of my career, I have decided that I would prefer to work and lecture in Jerusalem, and no longer wish to make the daily commute to Tel Aviv.{{HI_END}}
#. {{HI_BEGIN}}Because Jerusalem has a lot more IT jobs than hi-tech, it makes sense for me to "convert" to the IT field, where I can make just as strong a contribution as I did in hi-tech.{{HI_END}}
#. {{HI_BEGIN}}Bottom line, when hiring someone for a project that has a duration of more than a few months, the most critical requirements are his level of understanding of core programming and architectural concepts, and _not_ whether he needs an extra week or two to come up to speed in order to become familiar with the specific S/W toolchain. For example, note my recent smashing success in the field of [bioinformatics]({% if ABBREV %}{{CV_FULL}}{% endif %}#syntezza), where even though I had absolutely zero background in the fields of bioinformatics, biology, biochemistry, and genetics, still within 3 months I invented a solution for the client that saved them from bankruptcy.{{HI_END}}
%% endif
{{NL}}
...
%% macro DETAILS(id)
{% if ABBREV %} See [details]({{CV_FULL}}#{{id}}).{% endif %}
%% endmacro

%% macro JOB(id,title,company,details_flag=true)
{{NL}}###{% if ABBREV %}#{% endif %} {{title}} { #{{id}} }

_\@{{company}}{% if FULL %}_{% elif details_flag %}:_ {{DETAILS(id)}}{% else %}_{% endif %}
%% endmacro
...
{{KEYS}} cybersecurity, architect, algorithms, obfuscation, compiler, {{HI_CPP}}, javascript, WASM
...
{{ JOB(
	...
	'[Viaccess-Orca](https://www.viaccess-orca.com/), Ra\'anana - a subsidiary of \@Orange France, and \@Discretix/[Sansa Security](https://www.geektime.com/2015/07/30/arm-acquires-israeli-sansa-security-to-protect-iot-and-cellular-devices/), Netanya recently acquired by \@ARM',
	false
) }}
{{KEYS}} cybersecurity, DRM, architect, algorithms, anti-reverse engineering, obfuscation, LLVM compiler, cryptography, data science, fuzzy logic, {{HI_CPP}}, Python, TCL, bash, Java, Android root detection, IOS, Linux, ELF binary editor

#. The product was an [Over-The-Top (OTT)](https://en.wikipedia.org/wiki/Over-the-top_content) Internet pay TV system. We provided the S/W infrastructure to our customers, the legacy (i.e. satellite and cable) pay TV operators, so they could also provide an OTT service to their subscribers in order that they could try to compete with  [Netflix](https://www.netflix.com/il-en/). The system was designed for small screen Android and IOS devices, i.e. up to 10 inches. We used [DRM](https://en.wikipedia.org/wiki/Digital_rights_management) to encrypt the content.
%% if FULL
Originally the DRM was Microsoft's [PlayReady](https://en.wikipedia.org/wiki/PlayReady), and later on their own proprietary in-house DRM, and finally also Google's [Widevine](https://www.widevine.com/wv_drm.html).
%% endif
#. I was responsible for security policy and security architecture. I worked closely with the product management and the S/W development team leader in order to determine security requirements, their costs and benefits, and was the architect of their implementation. In many cases the security features were very complex, so I first needed to create a working proof-of-concept, before finalizing their specifications.
#. Originally the exclusive focus of security was protecting the devices from leaking content and keys, i.e. from being reverse engineered. We relied heavily on the premise that we refused to play on "rooted" Android devices or "jail-broken" IOS devices.
#. But as time went by, rooted Android devices became inexpensive and ubiquitous in the consumer market. Therefore due to declining royalties, the major studios (e.g. Disney, Sony, Warner Bros., etc.) were economically forced to allow playback on rooted devices. Therefore additional security had to implemented on the back-end web servers, e.g. to check whether or not a subscriber downloaded an unusually high number of hours of content, or whether the subscriber had simultaneous downloads from different IP addresses. I designed a secure and efficient data logging system. We logged data in order to better understand how subscribers were using the system, and in order to detect piracy. With tens of millions of subscribers, we collected a huge amount of data. I worked with data scientists to design "big data" collection and analysis techniques. And there was the economic challenge to minimize the communication costs of the data collection program.
#. I specified the anti-reverse engineering and [obfuscation](https://en.wikipedia.org/wiki/Obfuscation_(software)) programming frameworks and libraries in C/C++.
#. My typical development methodology was to first build a prototype for desktop Linux, secondly as a standalone [CLI](https://en.wikipedia.org/wiki/Command-line_interface) application on the target device, and finally to hand over a working prototype to the development team. Whenever possible I preferred to test on virtual machines.
...
#. All secure code modules on the device were implemented as native libraries written in C/C++. Typically offline utilites were implemented in Python.
#. I developed a lightweight obfuscated cryptographic library implemented as a H file using `inline` functions so that every module that included it had its own private copy of the library with a module specific randomized implementation which prevented an attack against a single core cryptographic module that could potentially subvert the whole application. Due to performance reasons, there were many cases where we could not afford to use AES, especially during the performance critical movie playback which itself relied upon AES decryption, therefore in these cases we used lightweight algorithms instead, e.g. [Xorshift PRNG](https://en.wikipedia.org/wiki/Xorshift) [stream cipher](https://en.wikipedia.org/wiki/Stream_cipher).
#. I developed an Android root detection mechanism using [fuzzy logic](https://en.wikipedia.org/wiki/Fuzzy_logic) techniques.
#. I developed a light weight dynamically randomized method to efficiently shroud all system calls so that their address is calculated just-in-time in registers before the call is made. The technique fooled the professional reverse engineering debuggers [Hex-Rays](https://www.hex-rays.com/) and [OllyDbg](https://en.wikipedia.org/wiki/OllyDbg) which normally can automatically identify and place anchors on the system calls.
#. I created a prototype of a [dynamic shared library (DSO)](https://en.wikipedia.org/wiki/Library_(computing)) that formally exported no symbols. In fact it used an asynchronous back channel that allowed the DSO to communicate with its caller by using a function declared with the [gcc constructor attribute](https://stackoverflow.com/questions/2053029/how-exactly-does-attribute-constructor-work) that executes before `dlopen()` returns. Typically this technique could be applied to _binary_ DSOs from our 3rd party vendors _without their knowledge_, by doing binary object file "surgery" (i.e. editing). We were faced with the challenge of securing a critical 3rd party library, i.e. the video [codec](https://en.wikipedia.org/wiki/Codec), which did not incorporate any obfuscation and where the vendor would not allow us to see or to suggest any modifications to their proprietary source code. They produced a single "one size fits all" library for all of their clients. Without securing this library, it was an ideal attack vector.
#. I developed vector operations for the C preprocessor that allowed a stream cipher to be applied to a constant string _pre-compile_ time that was used to shroud function name strings that were dynamically loaded using `dlsym()`.
#. I was responsible for the purchase decisions and usage policy of 3rd party obfuscation and cryptographic utilities and libraries. The two main 3rd party utilities that we used were the InterTrust WhiteCryption [SCP](https://www.intertrust.com/products/application-shielding/code-protection/) obfuscating C/C++ compiler and their [SKB](https://www.intertrust.com/products/application-shielding/secure-key-box/) "whitebox" cryptographic library{% if FULL %}. I worked with the WhiteCryption team to specify new features for their tools. For example, even though their [UPX-like](https://en.wikipedia.org/wiki/UPX) tool statically encrypts and packs a DSO, it automatically decrypts it when it is loaded - allowing anyone to load it including an attacker who wants to do reconnaissance, so we added a password protection mechanism using a global operating system object, i.e. an environment variable or a file{% endif %}.
%% if ABBREV
#. See [more details]({{CV_FULL}}#viaccess).
%% else
#. I developed a post-processor to obfuscate binary object ELF files.
#. I developed a background watchdog security thread to dynamically ensure that the binary code had not been tampered with.
#. I developed vector operations for the C preprocessor that allowed a stream cipher to be applied to a constant string _pre-compile_ time that was used to shroud function name strings that were dynamically loaded using `dlsym()`.
#. I was responsible for the purchase decisions and usage policy of 3rd party obfuscation and cryptographic utilities and libraries. The two main 3rd party utilities that we used were the InterTrust WhiteCryption [SCP](https://www.intertrust.com/products/application-shielding/code-protection/) obfuscating C/C++ compiler and their [SKB](https://www.intertrust.com/products/application-shielding/secure-key-box/) "whitebox" cryptographic library. I worked with the WhiteCryption team to specify new features for their tools. For example, even though their [UPX-like](https://en.wikipedia.org/wiki/UPX) tool statically encrypts and packs a DSO, it automatically decrypts it when it is loaded - allowing anyone to load it including an attacker who wants to do reconnaissance, so we added a password protection mechanism using a global operating system object, i.e. an environment variable or a file.
#. One of the most difficult architectural problems was to patch/update the application in a timely manner once a subscriber had already downloaded it. The final application was created by our customers, i.e. the satellite/cable operators. We supplied them with core libraries only. Therefore we did not have access to the enterprise keys from the Google Play Store or Apple App Store that were required to automatically push an update, e.g. to modify a configuration file with a new password due to a security breach on a server. The original security architect naively assumed that we would achieve higher security by building a customer specific version of our core module where we "baked" (i.e. embedded) in the customer's keys instead of using a separate customer specific configuration file to be dynamically loaded upon each invocation of the application. The customers were justifiably "paranoid" about demanding a very high level of quality assurance before making a release, because a single major bug could swamp them with tens of thousands of support calls where the cost of handling a single call could be greater than the monthly subscription fee. Typically their release cycle was every 6-12 months. Therefore the customers would never dare allow us to automatically push code to their subscribers that they did not yet vet. Eventually I architected a technical solution that allowed us to automatically push configuration files to the subscribers where we would givJOBe the customers 7 days prior notice. The solution also required that the servers support two sets of passwords and/or subscriber credentials during a key transition period.
%% endif
...
%% if FULL
#. The client publishes a web site that "outs" (i.e. exposes) terrorists.
#. Their WordPress web site was regularly attacked. And for reasons of personal physical safety, their management and researchers needed to remain anonymous.
#. Their enemies were well funded NGOs and government sponsored organizations.
#. Their original web site developers were not cybersecurity aware, and their researchers who often worked remotely (i.e. outside of the office) were typical computer users who were naive about cybersecurity and how to protect their anonymity.
#. After doing an initial risk assessment, my task was to implement graduated policies that improved their security without causing an upheaval to the way they worked.
%% endif


%% if not DD
	%% set title = '2015-15: Part-time: Startup: CTO & S/W Architect: Biomed: Gait Monitoring App'
%% else
	%% set title = '2015-15: Part-time: Startup: S/W Architect: Biomed: Gait Monitoring App'
%% endif
{{ JOB(
	'gait-monitoring',
	title,
	'Self-Startup (defunct), Jerusalem'
)}}
{{KEYS}} {% if not DD %}CTO, {% endif %}biomed, architect, android

%% if FULL
#. The simple form of the product was to provide a home diagnostic smartphone app to monitor gait (i.e. walking form). Gait problems often indicate underlying neurological problems, and are a precursor to falls which in the elderly can have serious and even fatal consequences. Gait can be monitored using the accelerometer and gravitation sensor built-in to most smartphones.
#. The complex form was to detect when the peroneal nerve around the knee "misfires" causing "foot drop". For many people this problem can be corrected by _external non-invasive_ low voltage electrical stimulation to the skin directly above the nerve. _FDA approved_ devices that use this technique cost thousands of dollars. But combined with a smartphone's built-in accelerometer and gravitation sensor which would detect an impending fall, the extra H/W required to apply the external voltage costs less than $10. We could use H/W similar to that used in adult sexual toys used to provide external low voltage electrical stimulation to the genitals - which BTW do _not_ require FDA approval. Our problem was that the cost of FDA approval for this _external non-invasive_ peroneal nerve stimulation device could cost more than $250K, where the high cost is associated with medically supervised clinical trials. (The FDA is in the _process_ of "fast tracking" many smartphone health apps, but as of 2017 no [small inventors](http://fortune.com/2017/09/26/fda-digital-health-apple-fitbit/) have yet to become eligible). We debated whether to market the system out of China (e.g. using Alibaba) to avoid FDA problems.
#. I spawned the idea after noticing a colleague at the gym who wore one of these expensive devices because he had degenerative peroneal nerve disease. We collaborated with an orthotics specialist who sells these devices and a MD neurologist who does geriatric research.
%% endif


%% if not DD
	%% set title = '2010-11: VP R&D: Transportation: Urban Traffic Vehicle Route Guidance Algorithms'
%% else
	%% set title = '2010-11: S/W Architect & Developer: Transportation: Urban Traffic Vehicle Route Guidance Algorithms'
%% endif
{{ JOB(
	'telequest',
	title,
...
{% if FULL %}#. {% endif %}I designed and implemented algorithms along with a computational infrastructure for urban traffic vehicle route guidance similar to what [Waze](https://www.waze.com/) does today.
%% if FULL
#. I coded in Java. I implemented the simulation infrastructure on the cloud on [AWS](https://aws.amazon.com/).
#. The simulations showed that with a guided vehicle penetration rate of just 15%, a very accurate representation of traffic flow could be achieved.
#. Also the simulations showed that opportunistic guidance algorithms created secondary traffic jams after the penetration rate exceeded 20%. And when the penetration rate exceeded 40%, opportunistic algorithms generated more traffic congestion than using no guidance at all.
#. Therefore an optimal algorithm had to anticipate traffic flows according to time of day (along with other factors), and to allocate vehicles with similar sources and destinations to different routes in order to avoid secondary traffic jams.
#. The Java implementation challenge was cost efficiency because the simulations used huge amounts of RAM and ran continuously for days on expensive servers. I could not rely on the efficacy of the built-in memory garbage collector (GC), so I had to explicitly coerce memory allocations and frees.
%% endif
...
{{ JOB(
	'agritech',
	'2009-12: Part-time: CTO: US Agricultural Trading Company with Nigeria',
	'Anonymous (defunct), US'
)}}
{{KEYS}} CTO, agritech, electric utility, telephony, economics, business development, negotiator
...
{{ JOB(
	'china',
	'2009-09: Part-time: Consultant: "Transparent" Technical Negotiator of Restricted Hi-Tech Trade with C',
	'Anonymous (defunct), US'
)}}
{{KEYS}} hi-tech, business development, negotiator
...
{{ JOB(
	'syntezza',
	'2009-09: S/W Architect & Developer: Bioinformatics: Invented Algorithm To Overcome PCR Inhibiition',
	'[Syntezza Molecular Detection](http://www.aneeva.com/wp-content/uploads/2013/06/syntezza-Syntezza-Moleclular.pdf) (defunct), Jerusalem'
) }}
{{KEYS}} bioinformatics, PCR, algorithms, architect, mathematical programming, data science, AI, {{HI_C}}, Python
...
%% if AGENT != 'wizedom'
	%% set details_flag = true
%% else
	%% set details_flag = false
%% endif
{{ JOB(
	'nds',
	'2004-09: NDS: Cybersecurity Researcher for a CA Satellite Pay TV System',
	'[Cisco-NDS](https://www.cisco.com/c/en/us/about/corporate-strategy-office/acquisitions/nds.html), Jerusalem',
	details_flag
) }}
{{KEYS}} cybersecurity, DRM, algorithms, cryptography, anti-reverse engineering, obfuscation, LLVM compiler, VM, QEMU, RPC, automated testing, S/W quality, {{HI_CPP}}, TCL, Python, Linux, bash, Win32
...
#. I developed techniques using Virtual Machine (VM) technology to crack [Digital Rights Management (DRM)](https://en.wikipedia.org/wiki/Digital_rights_management) schemes.
#. I developed a technique to subvert a commonly used class of [random number generators (RNG)](https://en.wikipedia.org/wiki/Random_number_generation) that are seeded with the [X86 CPU clock cycle counter (RDTSC)](http://x86.renejeschke.de/html/file_module_x86_id_278.html) [^rdtsc]. Many cryptographic algorithms rely upon an RNG for their initialization "handshake". I implemented this technique by hacking the open source [QEMU](https://www.qemu.org/) VM emulator written in C. This attack enabled subscriber credential sharing.
#. I wrote the technical specification for CCTV (Chinese government TV) to secure the TV feed of the 2008 Beijing Olympic broadcasts against international piracy. We almost won the contract, until Microsoft offered to do it for free.
#. I wrote business development reports about applying NDS security technology (1) to protect computer games against hacking, and (2) to protect inkjet printers from using 3rd party cartridges.
#. I arranged for well known security experts to give lectures and seminars at the company. The most successful and well attended course was a one week seminar on reverse engineering X86 assembly code using [OllyDbg](https://en.wikipedia.org/wiki/OllyDbg). The course was given by [Kris Kaspersky](https://www.linkedin.com/in/nezumi) who today ({{YEAR}}) works at Check Point. Afterwards many of us in the security group spent a number of months challenging each other with anti-reverse engineering riddles. The strongest techniques that I developed were (1) jumping into operands of long instructions that were designed to be short opcodes, (2) dynamic creation of opcodes in the heap, and (3) creation of obfuscated FORTH-like virtual machines.
...
[^rdtsc]: Today ({{YEAR}}), it is preferable to use Intel's built-in H/W RNG instruction [RDRAND](https://en.wikipedia.org/wiki/RdRand).


%% if not DD
	%% set title = '2002-03: CTO & S/W Architect: Accessibility: Invented System to Allow Blind to "See" Sonic Maps'
%% else
	%% set title = '2002-03: S/W Architect & Developer: Accessibility: Invented System to Allow Blind to "See" Sonic Maps'
%% endif
{{ JOB(
	'virtouch',
	title,
	'[Virtouch](https://www.israel21c.org/graphic-breakthrough-for-the-blind-developed-in-israel/) (defunct), Jerusalem'
) }}
{{KEYS}} {% if not DD %}CTO, {% endif %}accessibility, blind, architect, algorithms, GIS, MapML, HTML, SVG, javascript, XSLT, XML Schema, XSLT, {{HI_C}}, TCLNDS

%% if FULL
#. I was the inventor and architect of a system that allowed the blind to to "see" geographic maps and digital images that were prepared using industry standard [GIS](https://en.wikipedia.org/wiki/Geographic_information_system) map descriptions such as [MapML](https://maps4html.github.io/MapML/spec/).
#. Maps were displayed on a standard HTML browser using HTML, SVG, javascript, XSLT, combined with audio feedback.
#. A graphics tablet and stylus were used instead of a mouse because the blind can use a tablet much more effectively to navigate the screen compared to a mouse. (Today, {{YEAR}}, a touch tablet device could be used). The blind have their own sense of hand-eye coordination, and intuitively understand the stylus position on the tablet.
...
{{ JOB(
	'vyyo',
	'1999-2002: Vyyo: S/W Mngr & Architect: Network: Embedded & Offline Utilities for a "Wireless" Cable Modem and Router System',
	'[Vyyo](https://www.wirelessdesignonline.com/doc/phasecom-changes-name-to-vyyo-0001) (defunct), Jerusalem',
	details_flag
) }}
{{KEYS}} network, architect, algorithms, SNMP, SNMP-agent, NMS, automated testing, {{HI_C}}, TCL, embedded

%% if FULL or AGENT == 'wizedom'
#. I was the architect of the [SNMP](https://en.wikipedia.org/wiki/Simple_Network_Management_Protocol) network management system (NMS), [MIB](https://en.wikipedia.org/wiki/Management_information_base), and embedded SNMP agent{% if FULL %}. The workstation NMS was implemented in TCL, and the embedded SNMP agents on the cable modem and router were implemented in C{% endif %}.
#. I was the architect of a hybrid IP connection for cable modems where there was no physical cable upstream channel. Instead the upstream channel used a telephone modem (ATA), while the downstream channel used the cable modem{% if FULL %}. Head end network equipment for both interfaces was supplied by the cable operator. My solution was to dynamically modify the [arp table](https://en.wikipedia.org/wiki/Address_Resolution_Protocol) of the [edge router](http://searchnetworking.techtarget.com/definition/edge-router){% endif %}. For typical surfing, the effective downstream rate was as fast as a pure cable solution{% if FULL %}. The server implementation was in TCL, and the embedded cable modem implementation was in C{% endif %}. The company applied for a provisional patent.
...
#. I designed a _virtual_ testing lab with 64K modems and 512K PCs by multiplexing the _physical_ connections. The test lab had only 256 _physical_ cable modems, {% if ABBREV %}and {% endif %}4 _physical_ PCs with 8 network connections each{% if FULL %}, 1 cable router, and 2 24-port layer-2 programmable switches. By dynamically editing the PC MAC addresses, and by dynamically editing the MAC filters on the network switches, I was able to fool the router into believing that it faced 64K modems, and to fool the modems into believing that each one was shared by 16 PCs. The Linux workstation implementation was in TCL and C{% endif %}.
#. I designed a very efficient [hash table](https://en.wikipedia.org/wiki/Hash_table) algorithm in C for the router's [arp table](https://en.wikipedia.org/wiki/Address_Resolution_Protocol) cache, based upon an algorithm I had invented 10 years earlier. The special features of the hash table algorithm were no use of dynamic memory allocations for embedded safety, a unique 2^N table size algorithm that required no use of modulo/division operations for efficiency, and a [LIFO](https://techterms.com/definition/lifo) queue in order to gracefully handle table overflow.
#. I designed a flash memory file system for the modem and router{% if FULL %} in C{% endif %}.
#. I designed a [TLV](https://en.wikipedia.org/wiki/Type-length-value) configuration file utility both offline for the PC workstation, and embedded on the modem and router{% if FULL %} in C and TCL{% endif %}.
%% endif
...
{{ JOB(
	'jolt',
	'2001-01: Part-time: Consulting S/W Architect & Developer: Network Management System (NMS) for a FSO Device',
	'MRV-Jolt (defunct), Jerusalem'
) }}
{{KEYS}} network, architect, SNMP, SNMP-agent, NMS, Java, {{HI_C}}, TCL
%% endif
{{NL}}
...
%% if FULL
{{HI_BEGIN}}I performed a one week requirements study in order to select the most appropriate NMS infrastructure. I saved them over $200K compared to their original selection.{{HI_END}}
%% endif
...
{{ JOB(
	'zamir',
	'2000-01: Part-time: S/W Developer: Communications: Win32 Asynchronous TCP/IP DLL for a Visual Basic Project',
	'[Inex-Zamir](http://inextechnologies.com/), Jerusalem'
) }}
{{KEYS}} TCP/IP communications, {{HI_C}}, Visual Basic, Win32, soft real-time
%% endif
{{NL}}
...
{{KEYS}} gcc C compiler, architect, algorithms, DSL, {{HI_CPP}}, FORTH, LISP, TCL
...
{{ JOB(
	...
	'[Newport-Optimet](http://www.optimet.com/), Jerusalem'
) }}
{{KEYS}} measurement workstation, architect, algorithms, DSL, {{HI_C}}, TCL, OpenGL, Win32, soft real-time
...
{{ JOB(
	...
	'[Mer Group](http://www.mer-group.com/contact-map), Jerusalem',
	false
) }}
{{KEYS}} lecturer, Win32, {{HI_C}}


%% if FULL
{{ JOB(
	'ultramind',
	'1996-96: Part-time: Consulting S/W Engineer: Win32 Improve Performance of a Soft Real-Time Biofeedback Application',
	'[MindLife-UltraMind](http://hms.mindlife.co.il/), Jerusalem'
) }}
{{KEYS}} Win32, soft real-time, {{HI_C}}
%% endif
{{NL}}

%% if FULL
I greatly improving the performance and responsiveness of the customer's relaxation/meditation S/W application by tweaking how their application used the Win32 SDK. But more important, I developed my own meditation technique that uses simple S/W or even no S/W at all, and can easily be taught to most people within 5 minutes. Already in the 1960's [brainwave entrainment](https://en.wikipedia.org/wiki/Brainwave_entrainment) experiments showed that the brainwaves of cats, for examples, could be controlled by a strobe light source as long as the strobe frequency was within the normal frequency range of the cat's brain. Now human meditation takes place when the brain is exhibiting [theta waves](https://en.wikipedia.org/wiki/Brain_activity_and_meditation) which are in the range of 4-8 Hz. Note that this frequency range is the equivalent of a metronome operating at frequency of 240-480 beats per minute (bpm). Normally mechanical metronomes don't operate at speeds higher than 180 bpm, but today ({{YEAR}}) there are many free digital metronome applications (on smartphones, tablets, and desktop PCs) that operate at much higher speeds, where 240-360 bpm is the "sweet spot" for meditation. My favorite application is [GNU GTick](http://www.antcom.de/gtick/). I use the following 3 settings:
...
{{ JOB(
	'cefar',
	'1996-96: Part-time: Consulting S/W Engineer: Win32 Device Driver for a Frame Grabber',
...
%% if not DD
	%% set title = '1995-96: Elop: CTO & Architect: US DOD Mil-Spec Automated Testing: Night Hawk Fire Control System'
%% else
	%% set title = '1995-96: Elop: S/W Architect & Developer: US DOD Mil-Spec Automated Testing: Night Hawk Fire Control System'
%% endif
{{ JOB(
	'elop',
	title,
	'Pitkha Outsourcing (defunct), Jerusalem for \@[Elbit-Elop](http://elbitsystems.com/about-us-quality-assurance-elbit-systems-electro-optics-elop/), Rechovot'
) }}
{{KEYS}} {% if not DD %}CTO, {% endif %}automated testing, mil-spec, architect, DSL, {{HI_CPP}}, lex/yacc BASIC compiler, Win32, soft real-time
...
#. The standard tests were presented via a GUI which in fact emitted BASIC script.
...
%% if not DD
	%% set title = '1991-94: DSPG: CTO & S/W Architect: VLSI: Simulator & S/W Toolchain For DSPG PINE CPU'
%% else
	%% set title = '1991-94: DSPG: S/W Architect & Developer: VLSI: Simulator & S/W Toolchain For DSPG PINE CPU'
%% endif
{{ JOB(
	'dspg',
	title,
	'Pitkha Outsourcing (defunct), Jerusalem for \@[DSP Group](https://www.dspg.com/), Givat Shmuel'
) }}
{{KEYS}} {% if not DD %}CTO, {% endif %}VLSI simulator, S/W Development Toochain, architect, algorithms, DSL, {{HI_CPP}}, lex/yacc, assembly, Win32

%% if FULL
#. I was the S/W architect of a __clock accurate__ DSP CPU simulator along with a complete software development toolchain, i.e. a debugger, C compiler, assembler and linker. Note that the system was developed just _before_ the GNU Compiler Collection framework reached maturity, i.e. v2.95.
...
#. The technological breakthrough was my design of a [domain specific language (DSL)](https://en.wikipedia.org/wiki/Domain-specific_language) that described the CPU architecture __including the pipeline__. Implementation was in [lex/yacc](https://en.wikipedia.org/wiki/Yacc) and C++.
...
{{JOB(
	...
	'[Digital Equipment Corporation (DEC)](https://en.wikipedia.org/wiki/Digital_Equipment_Corporation) (defunct), Herzliya for \@[Iscar](http://www.iscar.com/newarticles.aspx/countryid/1/newarticleid/225), Tefen'
) }}
...
#. We used the language to configure the factory. We created a GUI which emitted CLI script. But major updates to the database were implemented via very large CLI scripts of tens of thousands of lines.
...
{{ JOB(
	...
	'Cubital (defunct), Herzliya - a charity project funded by the company and their CEO [Itzhak Pomerantz](https://il.linkedin.com/in/itzhak-pomerantz-91500b3) in cooperation with the [Lowenstein Rehabilitation Hospital](http://en.israel-clinics.guru/clinics/lowenstein_hospital_rehabilitation_center), and the IDF Rehabilitation Unit'
) }}
...
{{ JOB(
	...
	'[Cubital](https://en.wikipedia.org/wiki/Solid_ground_curing) (defunct), Herzliya'
) }}
{{KEYS}} 3D printing, {{HI_C}}, sysadmin, VAX/VMS
...
{{KEYS}} data collection terminal, {{HI_C}}, RS232, Z80, embedded, real-time
...
{{KEYS}} sewing workstation, DSL, algorithms, AutoCad, {{HI_C}}, awk, PC-DOS
...
{{KEYS}} word processor, {{HI_C}}, sysadmin, VAX/VMS


{{ JOB(
	'elta',
	'1983-84: Elta/IAI: S/W Developer: Real-Time: Data Collection Terminal & Radar for Lavi Fighter Plane',
	'DSI (defunct), Givatayim for \@[Elta/IAI](http://www.iai.co.il/17887-en/Groups_ELTA.aspx), Ashdod'
) }}
...
{{ JOB(
	...
	'[Mitre Corp](https://www.mitre.org/), McLean VA'
) }}
...
{{ JOB(
	...
	'[Ontario Energy Board (OEB)](https://www.oeb.ca/), Toronto'
) }}
...
#. I took over the [marginal cost pricing](https://en.wikipedia.org/wiki/Marginal_cost) (= peak load or time-of-day pricing) proposal of the [Ontario Hydro](http://www.opg.com/Pages/home.aspx) (i.e. at that time the name of Ontario's electric generation and transmission utility) economists who were forced to drop their case due to extreme political pressure from the metallurgy processing industry who at the time relied upon electric blast furnaces.
...
{% if FULL %}#. {% endif %}Today the field of computer science is changing so rapidly that one's formal education has a half-life of less than 5 years. Therefore in order to maintain my state-of-the-art professional edge, I am involved in an intensive effort of continuing education.{{DETAILS('continuing-education')}}
%% if FULL
...
#. I have eclectic interests. I regularly research new topics in depth.
...
## 5.0 Computer Languages, SDKs, and Operating Systems { #computer-languages }
...
## 6.0 Patents Under Development { #patents }
...
~~~ { #jinja2-cpp .c .numberLines }
/* !!! JINJA2 SYNTAX GOTCHYAS !!!
================================================================================
1. J2 statements and expressions should be wrapped in cpp macros,
    otherwise language sensitive editors found in many of the IDEs will choke on
    the source code. Therefore the cpp preprocessor must be run *before* the J2
    preprocessor.
2. However it can be an effective technique to include pure J2 source files by
    protecting them with an *external* cpp guard:
...
4. Cpp definitions must specify J2 delimiters *indirectly*,
    otherwise will cause J2 preprocessing errors. See lines 35-41.
5. Must *not* use the default J2 line comment '#' which conflicts with cpp.
6. Must *not* use J2 default expression delimiter, i.e. double braces,
    otherwise will conflict with C/C++ syntax.
================================================================================		
*/
...
#. My preferred GUI design is to have it generate CLI script - as opposed to directly invoking internal functions.
	* This allows GUI actions to be _semantically_ captured as opposed to capturing low level GUI events (e.g. individual mouse and keystroke events), and allows functional testing to be independent of the GUI.
	* And it allows large test and configuration scenarios to be first sketched with the GUI, and then to be extended with a text editor.
...
#. I design my code to minimize [complexity](https://en.wikipedia.org/wiki/Programming_complexity) in order to reduce bugs and make it easier to maintain. I analyze my code with the [cyclomatic complexity (McCabe)](https://en.wikipedia.org/wiki/Cyclomatic_complexity) and [Halstead complexity](https://en.wikipedia.org/wiki/Halstead_complexity_measures) metrics.
...
* __Generator:__ This document was generated using the [Pandoc](http://www.pandoc.org) universal document converter extended [Markdown](https://en.wikipedia.org/wiki/Markdown) engine, along with the [Jinja2](http://jinja.pocoo.org/) macro/template preprocessor. See the source code at my [github site]({{GITHUB_CV_DIR_URL}}).
...
* __Safety & non-annoyment pledge:__ This document is free of scripts, frames, advertisements, and animations.{% if WRITER == 'html5' %}<!-- This pledge was originally coined by David Pogue -->{% endif %}
%% endif
{{NL}}
