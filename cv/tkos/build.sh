...
sed -r -e 's/(<)(h[1-6]|li|p)(>)/\1\2 tabindex="0"\3/g' -e 's/<li id/<li tabindex="0" id/g' tmp/2.html > ${ROOT}-full.html
...
j2-cli.py $J2_COMMON -DWRITER=$WRITER tmp/ts.j2 defs.j2 $ROOT.md | trim-top.awk > tmp/$WRITER.md
pandoc.sh $WRITER --standalone tmp/metadata.yaml tmp/$WRITER.md > ${ROOT}-full.$WRITER
...
cp -fp ${ROOT}-full.html xfer/${OUT}-Full.html
cp -fp ${ROOT}-abbrev.html xfer/${OUT}-Abbrev.html
cp -fp ${ROOT}-full.docx xfer/${OUT}-Full.docx
cp -fp ${ROOT}-abbrev.docx xfer/${OUT}-Abbrev.docx
touch --reference=${ROOT}-full.html xfer/${OUT}-Abbrev.html xfer/${OUT}*.docx tmp/*.vcf
cp -fp *.vcf.j2 xfer
